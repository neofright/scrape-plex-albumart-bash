...
function scrape_plex_album_art()
{
    ...
    done < <(sqlite3 "$plex_db" "SELECT id,hash,user_thumb_url FROM metadata_items WHERE metadata_type = '9' AND library_section_id = '$library_section_id'")
}

## Your Plex library name must contain the string "Music".
## There is enough example code here to easily change this if your libraries are named differently.
while IFS="|" read -r library_id library_name
do
    printf "Scraping artwork from Plex library '%s'...\n" "$library_name"
    scrape_plex_album_art "$library_id"
done < <(sqlite3 "$plex_db" "SELECT id,name FROM library_sections WHERE name LIKE '%Music%' AND section_type = '8'")